...
class Repository {
    ...
    fun getWeatherDetail(query: String): MutableLiveData<Result> {
        ...
        NetworkUtil().api().getWeatherByName(query)
            .enqueue(object : Callback, retrofit2.Callback<WeatherResponse> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful)
                        ...
                        data.postValue(
                            response.body().let { Result(false, response.message(), null) })
    ...
    fun getWeatherDetailByCoordinate(lat: Double, lon: Double): MutableLiveData<Result> {
        data = MutableLiveData()
        NetworkUtil().api().getWeatherByCoordinate(lat, lon)
            .enqueue(object : Callback, retrofit2.Callback<WeatherResponse> {
                override fun onResponse(
                    call: Call<WeatherResponse>,
                    response: Response<WeatherResponse>
    ...
    fun getWeatherPrediction(city: String): MutableLiveData<Result> {
        data = MutableLiveData()
        NetworkUtil().api().getWeatherPrediction(city, 4)
            .enqueue(object : Callback, retrofit2.Callback<WeatherPredictionResponse> {
                override fun onResponse(
    ...
    fun getCity(cityName: String, country: String, context: Context) =
        CityDb.getInstance(context).getCityDao().getCityByNameAndCountry(cityName, country)

    suspend fun deleteCity(city: City, context: Context) {
        CityDb.getInstance(context).getCityDao()
            .deleteCity(city.cityName.toString(), city.country.toString())
    }

    fun getAllCity(context: Context): LiveData<List<City>> =
        CityDb.getInstance(context).getCityDao().getAllCity()
}